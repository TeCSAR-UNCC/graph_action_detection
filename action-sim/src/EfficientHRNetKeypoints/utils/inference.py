...
def get_outputs(
    ...
    outputs = model(image)
    if cfg.LOSS.WITH_AE_LOSS[1] is True:
        heatmaps = outputs[-1][:, :cfg.DATASET.NUM_JOINTS]
        tags = outputs[-1][:, cfg.DATASET.NUM_JOINTS:]
    else:
        heatmaps = outputs[-2][:, :cfg.DATASET.NUM_JOINTS]
        tags = outputs[-2][:, cfg.DATASET.NUM_JOINTS:]
...
def aggregate_results(
    ...
    if scale_factor == 1 or len(cfg.TEST.SCALE_FACTOR) == 1:
        ...
        for tms in tags:
            tags_list.append(torch.unsqueeze(tms, dim=3))
