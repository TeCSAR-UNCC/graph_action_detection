...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self,cfg, **kwargs):
        ...
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.25,scale_factor)) * block.expansion))  for i in range(len(num_channels))
        ]
        if width_mult == 1:
            ...
            self.trans1_branch1 = nn.Sequential(nn.Conv2d(int(_make_divisible(24*width_mult)),int(math.ceil(32*pow(1.25,scale_factor))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans1_branch2 = nn.Sequential(nn.Conv2d(int(_make_divisible(40*width_mult)),int(math.ceil(64*pow(1.25,scale_factor))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))

        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels) #should happen automatically
        ...
        if width_mult == 1:
            ...
            self.trans2_branch1 = nn.Sequential(nn.Conv2d(int(_make_divisible(32*width_mult)), int(math.ceil(32*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans2_branch2 = nn.Sequential(nn.Conv2d(int(_make_divisible(64*width_mult)), int(math.ceil(64*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans2_branch3 = nn.Sequential(nn.Conv2d(int(_make_divisible(112*width_mult)), int(math.ceil(128*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))

        self.stage3, pre_stage_channels = self._make_stage(
            self.stage3_cfg, num_channels) #should happen automatically
        ...
        if width_mult == 1:
            ...
            self.trans3_branch1 = nn.Sequential(nn.Conv2d(int(_make_divisible(32*width_mult)), int(math.ceil(32*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch2 = nn.Sequential(nn.Conv2d(int(_make_divisible(64*width_mult)), int(math.ceil(64*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch3 = nn.Sequential(nn.Conv2d(int(_make_divisible(112*width_mult)), int(math.ceil(128*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch4 = nn.Sequential(nn.Conv2d(int(_make_divisible(320*width_mult)), int(math.ceil(256*pow(1.25,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(256*pow(1.25,scale_factor)))), nn.ReLU(inplace=True))
        
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, multi_scale_output=False) #should happen automatically
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            output_channels = int(math.ceil(deconv_cfg.NUM_CHANNELS[i]*pow(1.25, scale_factor)))
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        scale_factor = 0
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.25,scale_factor))))  for i in range(len(num_channels))
        ]
...
def get_pose_net(cfg, is_train, **kwargs):
    model = PoseHigherResolutionNet(cfg, width_mult=1, depth_mult=1, scale_factor=0, **kwargs)
