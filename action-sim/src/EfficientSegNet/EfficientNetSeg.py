...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.depth_mult = config.MODEL.DEPTH_MULT
        settings = [
            # t,  c, n, s, k
        ...
        # yapf: enable
        out_channels = _round_filters(32, config.MODEL.WIDTH_MULT)#width_mult)
        features = [ConvBNReLU(3, out_channels, 3, stride=2)]

        in_channels = out_channels
        for t, c, n, s, k in settings:
            out_channels = _round_filters(c, config.MODEL.WIDTH_MULT)#width_mult)
            repeats = _round_repeats(n, config.MODEL.DEPTH_MULT)#self.depth_mult)
        ...
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.2455,config.MODEL.SCALE_FACTOR)) * block.expansion))  for i in range(len(num_channels))
        ]
        if config.MODEL.WIDTH_MULT == 1 and config.MODEL.DEPTH_MULT == 1:
            ...
            self.trans1_branch1 = nn.Sequential(nn.Conv2d(int(_make_divisible(24*config.MODEL.WIDTH_MULT)),int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans1_branch2 = nn.Sequential(nn.Conv2d(int(_make_divisible(40*config.MODEL.WIDTH_MULT)),int(math.ceil(64*pow(1.2455,cfg.MODEL.SCALE_FACTOR))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))

        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels,config)
        ...
        if config.MODEL.WIDTH_MULT == 1 and config.MODEL.DEPTH_MULT == 1:
            ...
            self.trans2_branch1 = nn.Sequential(nn.Conv2d(int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR))), int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans2_branch2 = nn.Sequential(nn.Conv2d(int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR))), int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans2_branch3 = nn.Sequential(nn.Conv2d(int(_make_divisible(112*config.MODEL.WIDTH_MULT)), int(math.ceil(128*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))

        self.stage3, pre_stage_channels = self._make_stage(
            self.stage3_cfg, num_channels, config)
        ...
        if config.MODEL.WIDTH_MULT == 1 and config.MODEL.DEPTH_MULT == 1:
            ...
            self.trans3_branch1 = nn.Sequential(nn.Conv2d(int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR))), int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans3_branch2 = nn.Sequential(nn.Conv2d(int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR))), int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans3_branch3 = nn.Sequential(nn.Conv2d(int(math.ceil(128*pow(1.2455,config.MODEL.SCALE_FACTOR))), int(math.ceil(128*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))
            self.trans3_branch4 = nn.Sequential(nn.Conv2d(int(_make_divisible(320*config.MODEL.WIDTH_MULT)), int(math.ceil(256*pow(1.2455,config.MODEL.SCALE_FACTOR))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(256*pow(1.2455,config.MODEL.SCALE_FACTOR)))), nn.ReLU(inplace=True))

        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, config, multi_scale_output=True)
    ...
    def _make_stage(self, layer_config, num_inchannels, config,
        ...
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.2455,config.MODEL.SCALE_FACTOR))))  for i in range(len(num_channels))
        ]
