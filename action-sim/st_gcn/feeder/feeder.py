...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        #samples, channels,time,v=graph nodes, M= people
    ...
    def __getitem__(self, index):
        ...
        #the numpy must hold everything in a single array
    ...
    def top_k(self, score, top_k):
        rank = score.argsort()
        print(rank)
        hit_top_k = [l in rank[0] for i, l in enumerate(self.label)]
        return sum(hit_top_k) * 1.0 / len(hit_top_k)
    def top_eval(self, score, top_k):
        rank = score.argsort()
        print(rank)
        hit_top_k = [l in rank[0] for i, l in enumerate(self.label)]
        return sum(hit_top_k) * 1.0 / len(hit_top_k)
